...
public class Phonebook {
	...
	public boolean updatePhone(String id, String phoneId, String phone) {
	...
	public boolean deletePhone(String id, String phoneId) {
		Integer affected_rows = this.db.changeDBData("DELETE FROM `phone` WHERE `id`=" + phoneId);
		if (affected_rows > 0)
		{
			this.persons.get(id).getPhones().remove(phoneId);
			return true;
		}
		else
		{
	...
	public boolean addPhone(String id, String phone) {
		Integer affected_rows = this.db.changeDBData("INSERT INTO `phone` (`owner`, `number`) VALUES ('" + id +"', '" + phone + "')");
		if (affected_rows > 0)
		{
			this.persons.get(id).getPhones().put(this.db.getLastInsertId().toString(), phone);
			return true;
		}
		else
		{
